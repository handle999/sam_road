...
def test_graph_label_generator():
    ...
    if dataset == 'cityscale':
        rgb_path = './datasets/cityscale/20cities/region_166_sat.png'
        # Load GT Graph
        gt_graph = pickle.load(open(f"./datasets/cityscale/20cities/region_166_refine_gt_graph.p",'rb'))
        coord_transform = lambda v : v[:, ::-1]
    elif dataset == 'spacenet':
        rgb_path = 'datasets/spacenet/RGB_1.0_meter/AOI_2_Vegas_210__rgb.png'
        # Load GT Graph
        gt_graph = pickle.load(open(f"datasets/spacenet/RGB_1.0_meter/AOI_2_Vegas_210__gt_graph.p",'rb'))
        # gt_graph = pickle.load(open(f"datasets/spacenet/RGB_1.0_meter/AOI_4_Shanghai_1061__gt_graph_dense_spacenet.p",'rb'))
...
class SatMapDataset(Dataset):
    def __init__(self, config, is_train, dev_run=False):
        ...
        if self.config.DATASET == 'cityscale':
            ...
            if self.is_train:
                active_mask_pattern = './datasets/cityscale/20cities/region_{}_gt.png'
            else:
                # 验证集使用生成的残缺先验，确保指标稳定
                active_mask_pattern = './datasets/cityscale/sample_0.5/region_{}_refine_gt_graph_partial.png'
            # ==========================================
            ...
            if self.is_train:
                active_mask_pattern = './datasets/spacenet/RGB_1.0_meter/{}__gt.png'
            else:
                # TODO: sample percentage, use argument or config
                active_mask_pattern = './datasets/spacenet/sample_0.5/{}__gt_graph_partial.png'
            # ==========================================
        ...
        for tile_idx in tile_indices:
            ...
            # ==========================================
            # [UNIFIED 4-CHANNEL]: 所有数据集统一读取先验通道 (Active or GT)
        ...
        if not self.is_train:
            ...
            self.eval_patches = []
            # [MODIFICATION]: 使用实际加载进内存的图片数量 (len(self.rgbs)) 进行遍历
    ...
    def __getitem__(self, idx):
        ...
        # ==========================================
        # [UNIFIED 4-CHANNEL]: 所有数据集统一执行 Modality Dropout 和拼接
        # # 1. 训练阶段: 50% 概率将先验置零
        ...
        # ==========================================
        # [MODIFICATION 9: 进阶版 Prior Augmentation (先验增强)]
        if self.is_train:
            rand_val = random.random()
            
            if rand_val < 0.2:
                # 【20% 概率：全黑】
                # 迫使模型保持纯视觉生成 (Generate) 的底线能力
                active_mask_patch = np.zeros_like(active_mask_patch)
                
            elif rand_val < 0.8:
                # 【60% 概率：残缺/腐蚀】 (最核心的训练状态！)
                # 模拟轨迹中断、GPS 丢失。强迫模型顺着断头路，去 RGB 里找纹理连线。
                active_mask_patch = active_mask_patch.copy()
                h, w = active_mask_patch.shape
                # 随机生成 3 到 10 个遮挡块，把 GT 切碎
                for _ in range(random.randint(3, 10)):
                    ...
                    active_mask_patch[ey:ey+erase_h, ex:ex+erase_w] = 0
                    
            else:
                # 【20% 概率：完美 GT】
                # 让模型知道，有时候提示也是很准的，可以加速收敛
                pass
        elif not self.is_train:
            # 验证集由于是自动跑的，为了指标稳定，先给全黑 (闭卷)
            # 真正的半开卷我们会在 inferencer_copy.py 里手动传
            active_mask_patch = np.zeros_like(active_mask_patch)
        # ==========================================
