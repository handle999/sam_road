...
import addict
import json
# random only needed in dataset_4ch.py for modality dropout
...
class SatMapDataset(Dataset):
    def __init__(self, config, is_train, dev_run=False):
        ...
        if not self.is_train:
            ...
            self.eval_patches = []
            # Use len(self.rgbs) instead of len(tile_indices) because empty tiles are skipped
    ...
    def __getitem__(self, idx):
        ...
        # rgb: [H, W, 3] 0-255
        # masks: [H, W] 0-1
        # Note: For 4-channel (RGB+active) input, use dataset_4ch.py instead
        return {
