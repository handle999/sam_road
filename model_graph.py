...
from torch_geometric.utils import to_edge_index, unbatch
...
class TopoNet(nn.Module):
    def __init__(self, config, feature_dim):
        ...
        # +++ 新增 GNN (GAT) 层 +++
        # 使用2层GAT。第一层将原始特征维度映射到隐藏维度
        # 第二层在隐藏维度上操作
        self.gnn_heads = 4
        self.gnn1 = GATConv(feature_dim, self.hidden_dim, heads=self.gnn_heads, dropout=0.1)
        self.gnn2 = GATConv(self.hidden_dim * self.gnn_heads, self.hidden_dim, heads=1, concat=False, dropout=0.1)
        # +++ GNN层结束 +++
    ...
    # +++ 修改 forward 方法签名，增加 connected 用于处理图 +++
    def forward(self, points, point_features, pairs, pairs_valid, connected=None):
        # points: [B, N_points, 2]
        # point_features: [B, N_points, D]
        # pairs: [B, N_samples, N_pairs, 2]
        # pairs_valid: [B, N_samples, N_pairs]
        # #### add: connected: [B, N_samples, N_pairs] (仅训练时提供)

        batch_size, n_samples, n_pairs, _ = pairs.shape
        
        # +++ GNN 特征增强模块 (仅在训练时激活) +++
        if self.training and connected is not None:
            # 批处理GNN：为批次中的每个图独立进行GNN增强
            contextual_point_features_list = []
            for i in range(batch_size):
                # x 是当前图的节点特征: [N_nodes, D]
                x = point_features[i] 
                
                # 从 pairs 和 connected 构建 edge_index
                # item_pairs: [N_samples, N_pairs, 2], item_connected: [N_samples, N_pairs]
                item_pairs = pairs[i]
                item_connected = connected[i]

                # 找到所有为True的连接
                true_edges_mask = (item_connected == 1)
                
                # 如果没有真实边，则跳过GNN，使用原始特征
                if not true_edges_mask.any():
                    enhanced_x = F.relu(self.feature_proj(x))
                    contextual_point_features_list.append(enhanced_x)
                    continue

                # edge_index: [2, NumTrueEdges]
                edge_index = item_pairs[true_edges_mask].t()
                
                # GNN前向传播
                # 注意：这里的x是包含了padding的，但GATConv只会为edge_index中存在的节点进行消息传递
                # 对没有参与消息传递的节点，其特征等价于通过了一个线性变换
                x = F.dropout(x, p=0.1, training=self.training)
                x = self.gnn1(x, edge_index)
                x = F.elu(x)
                x = F.dropout(x, p=0.1, training=self.training)
                enhanced_x = self.gnn2(x, edge_index)
                
                contextual_point_features_list.append(enhanced_x)
            
            # 重新组合成批次张量
            point_features = torch.stack(contextual_point_features_list, dim=0)

        else:
            # 推理时或不提供图信息时，使用原始的线性投影
...
class SAMRoad(pl.LightningModule):
    ...
    # +-----------------+加入connected参数
    def forward(self, rgb, graph_points, pairs, valid, connected=None, ):
        # rgb: [B, H, W, C]
        ...
        # [B, N_sample, N_pair, 1]
        # #### add: parameters 'connected'
        topo_logits, topo_scores = self.topo_net(graph_points, point_features, pairs, valid, connected)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # [B, H, W, 2]
        # #### add: parameters 'connected'
        mask_logits, mask_scores, topo_logits, topo_scores = self(rgb, graph_points, pairs, valid, connected)
