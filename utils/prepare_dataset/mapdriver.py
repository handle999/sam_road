...
def GetMapAroundALoc(lat,lon, rangeInMeter, heading, folder = "googlemap/", start_lat = 42.1634, start_lon = -71.36, resolution = 1024, padding = 128, zoom = 19):
    ...
    # for i in xrange(ilat_min, ilat_max+1):      # xrange是python2的迭代器，python3被废弃，直接使用range遍历
...
def GetMapInRect(min_lat,min_lon, max_lat, max_lon , folder = "googlemap/", start_lat = 42.1634, start_lon = -71.36, resolution = 1024, padding = 128, zoom = 19, scale = 2):
    ...
    # for i in xrange(ilat_min, ilat_max+1):      # xrange是python2的代码吧，python3被废弃了，直接使用range就行
        # for j in xrange(ilon_min, ilon_max+1):
    for i in range(ilat_min, ilat_max + 1):
        for j in range(ilon_min, ilon_max + 1):
...
class OSMLoader:
    def __init__(self, region, noUnderground=False, osmfile=None, includeServiceRoad=False):
        ...
        # 将非法字符替换成合法形式
        safe_range = sub_range.replace(",", "_")
        safe_filename = f"map_bbox_{safe_range}.osm"
        filename = os.path.join(tmp_dir, safe_filename)
        ...
        if osmfile is None:
            while not os.path.exists(filename):
                # url = f"http://overpass-api.de/api/map?bbox={sub_range}"
                url = f"https://overpass.nchc.org.tw/api/map?bbox={sub_range}"  # hhy modify 2026-01-16 台湾的Overpass服务器，速度较快且稳定
                temp_file = f"temp_download_{safe_filename}"
                ...
                try:
                    ...
                    print("Wait for one minute before retrying...")
                    sleep(60)
