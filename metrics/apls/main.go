...
func main() {
	...
	if len(os.Args) > 4 {
		...
		if dataset == "cityscale" {
			// CityScale: 2k x 2k tiles
			fmt.Println("Using CityScale parameters (region size=2048)")
			interval_2 = 15.0 // 30 meters (15*2)
			interval_1 = int(interval_2 * 1.5)
			min_distance_filter = 100.0
			prop_step = 4
			margin_size = 100.0
			region_size = 2048.0
		} else {
			// SpaceNet / small tiles (region size=352)
			fmt.Println("Using SpaceNet parameters (region size=352)")
			interval_2 = 10.0 // 20 meters (10*2)
			interval_1 = int(interval_2 * 1.5)
			min_distance_filter = 30.0
			prop_step = 3
			margin_size = 30.0
			region_size = 352.0
		}
