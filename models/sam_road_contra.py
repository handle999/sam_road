...
########## MODIFICATION START ##########
# 描述: 新增一个简单的图编码器，用于将图结构信息编码为向量。
#      它通过节点特征和邻接关系（从pairs构建）来聚合信息。

class SimpleGATEncoder(nn.Module):
    def __init__(self, node_feature_dim, output_dim, n_heads=4):
        super().__init__()
        ...
        # 将节点特征投影到注意力机制所需的维度
        self.feature_proj = nn.Linear(node_feature_dim, output_dim)
        
        # 注意力权重计算层
        self.attn_proj = nn.Linear(2 * output_dim, n_heads)
        self.leaky_relu = nn.LeakyReLU(0.2)
        
    def forward(self, node_features, pairs, valid_pairs_mask):
        """
        Args:
            node_features (Tensor): [B, N_points, D_node], 节点的视觉特征.
            pairs (Tensor): [B, N_samples * N_pairs, 2], 定义了边的索引.
            valid_pairs_mask (Tensor): [B, N_samples * N_pairs], 标记有效边.
        
        Returns:
            Tensor: [B, output_dim], 代表整个图的编码向量.
        """
        B, N_points, _ = node_features.shape
        
        # 描述: 在使用 pairs 进行索引前，将其转换为 long (int64) 类型，解决 "Expected dtype int64 for index" 错误
        pairs = pairs.long()

        # 1. 节点特征投影
        projected_features = self.feature_proj(node_features) # [B, N_points, D_out]
        
        # 2. 构建邻接列表 (简化处理，只考虑一个大图)
        #    为了简化，我们将所有子图的边合并处理
        src_nodes = projected_features.gather(1, pairs[:, :, 0].unsqueeze(-1).expand(-1, -1, self.output_dim))
        tgt_nodes = projected_features.gather(1, pairs[:, :, 1].unsqueeze(-1).expand(-1, -1, self.output_dim))
        
        # 3. 计算注意力分数
        edge_features = torch.cat([src_nodes, tgt_nodes], dim=-1)
        attn_logits = self.attn_proj(edge_features) # [B, N_edges, N_heads]
        attn_logits = self.leaky_relu(attn_logits)
        
        # 使用掩码确保只在有效边上计算注意力
        valid_pairs_mask = valid_pairs_mask.unsqueeze(-1).expand_as(attn_logits)
        attn_logits.masked_fill_(~valid_pairs_mask, torch.finfo(attn_logits.dtype).min)
        
        attn_weights = F.softmax(attn_logits, dim=1) # [B, N_edges, N_heads]
        
        # 4. 聚合特征 (Aggregation)
        # 原始的错误代码:
        # graph_embedding = torch.mean(projected_features, dim=1) 
        
        # 描述: 使用计算出的注意力权重 attn_weights 来聚合节点特征。这是实现一个真正GAT的关键步骤。
        
        # 我们需要对源节点的特征进行加权求和。
        # src_nodes 的形状是 [B, N_edges, D_out]
        # attn_weights 的形状是 [B, N_edges, N_heads]
        # 为了聚合，我们将 src_nodes 扩展以匹配 head 的数量
        src_nodes_expanded = src_nodes.unsqueeze(-2) # -> [B, N_edges, 1, D_out]
        attn_weights_expanded = attn_weights.unsqueeze(-1) # -> [B, N_edges, N_heads, 1]
        
        # 执行加权求和
        # weighted_features: [B, N_edges, N_heads, D_out]
        weighted_features = src_nodes_expanded * attn_weights_expanded 
        
        # 在边的维度上求和，得到每个 head 的聚合结果
        aggregated_features_per_head = torch.sum(weighted_features, dim=1) # -> [B, N_heads, D_out]
        
        # 对所有 head 的结果进行平均，得到最终的图表示
        graph_embedding = torch.mean(aggregated_features_per_head, dim=1) # -> [B, D_out]
...
class SAMRoad(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        self.graph_encoder = SimpleGATEncoder(
            node_feature_dim=encoder_output_dim,
            output_dim=config.GRAPH_EMBED_DIM
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        ########## MODIFICATION START ##########
        # 描述: 计算视觉-图谱对比损失 (Vision-Graph Contrastive Loss)

        # 1. 获取全局视觉特征
        #    使用自适应平均池化将 [B, D, h, w] 的特征图转换为 [B, D] 的向量
        vision_features_global = F.adaptive_avg_pool2d(image_embeddings, (1, 1)).flatten(start_dim=1)
        
        # 2. 获取图谱特征
        #    a. 首先需要从 image_embeddings 中为每个图节点采样特征
        point_features = self.bilinear_sampler(image_embeddings, graph_points)
        #    b. 将节点特征和边信息送入图编码器，我们需要将 `pairs` 和 `valid` reshape 成适合图编码器的形式
        b, n_samples, n_pairs, _ = pairs.shape
        pairs_flat = pairs.view(b, -1, 2)
        valid_flat = valid.view(b, -1)
        graph_features_global = self.graph_encoder(point_features, pairs_flat, valid_flat)
        ...
        # 4. L2 归一化特征
        vision_embed = F.normalize(vision_embed, dim=-1, p=2)
        graph_embed = F.normalize(graph_embed, dim=-1, p=2)
        ...
        #    计算交叉熵损失
        contrastive_loss_v = F.cross_entropy(logits, labels)
        contrastive_loss_g = F.cross_entropy(logits.T, labels)
        contrastive_loss = (contrastive_loss_v + contrastive_loss_g) / 2.0
