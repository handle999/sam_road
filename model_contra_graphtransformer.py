...
########## MODIFICATION START ##########
# 描述: 一个全新的、感知几何信息的 Graph Transformer 编码器，用于替换 SimpleGATEncoder。
#      它将节点的(x,y)坐标作为位置编码，并使用注意力掩码来体现图的连接结构。

class GraphTransformerEncoder(nn.Module):
    def __init__(self, node_feature_dim, pos_dim, d_model, n_heads, n_layers, dropout=0.1):
        super().__init__()
        self.d_model = d_model

        # 1. 投影层：将输入的视觉特征和坐标投影到模型的工作维度(d_model)
        ...
        # 2. 标准的 Transformer Encoder 层
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=d_model,
            nhead=n_heads,
            dim_feedforward=d_model * 4, # 通常设为d_model的4倍
            dropout=dropout,
            activation='gelu',
            batch_first=True, # 输入格式为 [B, N, D]
            norm_first=True   # 提高训练稳定性
        )
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, num_layers=n_layers)

        # 3. Dropout 和 LayerNorm
        self.dropout = nn.Dropout(dropout)
        self.norm = nn.LayerNorm(d_model)

    def _create_attention_mask(self, pairs, num_nodes, valid_pairs_mask, device):
        """ 从边的列表创建注意力掩码 """
        B, N_edges = pairs.shape[0], pairs.shape[1]
        # 0代表可以attend，-inf代表不可以
        attn_mask = torch.full((B, num_nodes, num_nodes), float('-inf'), device=device)

        # 自己到自己的连接总是允许的
        diag_indices = torch.arange(num_nodes, device=device)
        attn_mask[:, diag_indices, diag_indices] = 0

        # 根据valid_pairs_mask筛选出有效的边
        batch_indices = torch.arange(B, device=device).unsqueeze(1).expand(-1, N_edges)
        valid_mask_flat = valid_pairs_mask.flatten()
        
        batch_indices_flat = batch_indices.flatten()[valid_mask_flat]
        src_indices_flat = pairs[:, :, 0].flatten()[valid_mask_flat]
        tgt_indices_flat = pairs[:, :, 1].flatten()[valid_mask_flat]

        # 设置有效边对应的位置为0，允许相互attend
        attn_mask[batch_indices_flat, src_indices_flat, tgt_indices_flat] = 0
        attn_mask[batch_indices_flat, tgt_indices_flat, src_indices_flat] = 0 # 无向图

        return attn_mask

    def forward(self, node_features, points, pairs, valid_nodes_mask, valid_pairs_mask):
        """
        Args:
            node_features (Tensor): [B, N_points, D_node], 节点的视觉特征
            points (Tensor): [B, N_points, 2], 节点的(x,y)坐标
            pairs (Tensor): [B, N_edges, 2], 边的索引
            valid_nodes_mask (Tensor): [B, N_points], 有效节点掩码 (True为有效)
            valid_pairs_mask (Tensor): [B, N_edges], 有效边掩码 (True为有效)
        
        Returns:
            Tensor: [B, d_model], 代表整个图的编码向量
        """
        B, N_points, _ = node_features.shape
        device = node_features.device

        # 1. 特征和位置编码
        projected_features = self.input_proj(node_features)
        positional_encoding = self.pos_encoder(points)
        x = projected_features + positional_encoding # 将位置信息注入特征
        x = self.dropout(self.norm(x))

        # 2. 创建注意力掩码和padding掩码
        # 注意力掩码：决定哪些节点之间可以相互传递信息（基于图结构）
        attn_mask = self._create_attention_mask(pairs.long(), N_points, valid_pairs_mask, device)
        # Padding掩码：告诉Transformer哪些是填充的节点，不应参与计算
        # PyTorch中, src_key_padding_mask的True代表是padding
        padding_mask = ~valid_nodes_mask 

        ########## MODIFICATION START ##########
        # 描述: 调整 attn_mask 的形状以适配多头注意力的要求。
        
        # 获取注意力头的数量
        n_heads = self.transformer_encoder.layers[0].self_attn.num_heads
        
        # 将掩码从 [B, N, N] -> [B, 1, N, N] -> [B, n_heads, N, N]
        attn_mask_expanded = attn_mask.unsqueeze(1).repeat(1, n_heads, 1, 1)
        
        # 最终形状为 [B * n_heads, N, N]
        attn_mask_reshaped = attn_mask_expanded.view(B * n_heads, N_points, N_points)
        # 确保掩码的数据类型与输入x一致，消除UserWarning
        # This is a good practice for compatibility.
        attn_mask_final = attn_mask_reshaped.to(x.dtype)

        # 3. 通过Transformer Encoder (使用调整后的掩码)
        transformer_output = self.transformer_encoder(x, mask=attn_mask_final, src_key_padding_mask=padding_mask)
        ########## MODIFICATION END ##########

        # 4. 聚合生成图表示 (只在有效节点上进行平均池化)
        transformer_output[padding_mask] = 0 # 将padding位置的输出清零
        valid_nodes_count = valid_nodes_mask.sum(dim=1, keepdim=True).clamp(min=1)
        graph_embedding = torch.sum(transformer_output, dim=1) / valid_nodes_count
...
class SAMRoad(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # 1. 实例化图编码器
        #    注意：这里的 node_feature_dim 必须与 BilinearSampler 采样的特征维度一致
        self.graph_encoder = GraphTransformerEncoder(
            node_feature_dim=encoder_output_dim,
            pos_dim=2, # 节点的(x,y)坐标
            d_model=config.GT_D_MODEL,
            n_heads=config.GT_N_HEADS,
            n_layers=config.GT_N_LAYERS
        )
        
        # 2. 定义视觉和图谱的投影头 (Projection Heads)
        #    它们将各自的特征投影到用于对比学习的共享空间
        self.vision_projector = nn.Sequential(
            nn.Linear(encoder_output_dim, encoder_output_dim),
            nn.ReLU(),
            nn.Linear(encoder_output_dim, config.CONTRASTIVE_EMBED_DIM)
        )
        self.graph_projector = nn.Sequential(
            nn.Linear(config.GRAPH_EMBED_DIM, config.GRAPH_EMBED_DIM),
            nn.ReLU(),
            nn.Linear(config.GRAPH_EMBED_DIM, config.CONTRASTIVE_EMBED_DIM)
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        ####### MODIFICATION START ##########
        # 4a. [冗余计算] 重新进行图像编码，获取 image_embeddings
        # [B, C, H, W]
        x = rgb.permute(0, 3, 1, 2)
        x = (x - self.pixel_mean) / self.pixel_std
        # [B, D, h, w]
        ...
        # 4b. 获取全局视觉特征
        vision_features_global = F.adaptive_avg_pool2d(image_embeddings, (1, 1)).flatten(start_dim=1)
        
        # 4c. 获取图谱特征
        # [冗余计算] 重新采样节点特征
        point_features = self.bilinear_sampler(image_embeddings, graph_points)
        
        # 为 GraphTransformer 准备输入
        b, n_samples, n_pairs, _ = pairs.shape
        pairs_flat = pairs.view(b, -1, 2)
        valid_flat = valid.view(b, -1)
        valid_nodes_mask = (graph_points.sum(dim=-1) != 0)

        # 调用新的 GraphTransformer
        graph_features_global = self.graph_encoder(
            node_features=point_features,
            points=graph_points,
            pairs=pairs_flat,
            valid_nodes_mask=valid_nodes_mask,
            valid_pairs_mask=valid_flat
        )

        # 4d. 通过投影头
        vision_embed = self.vision_projector(vision_features_global)
        graph_embed = self.graph_projector(graph_features_global)
        
        # 4e. L2 归一化和 InfoNCE 损失计算
        eps = 1e-8
        vision_embed = vision_embed / (torch.norm(vision_embed, p=2, dim=1, keepdim=True) + eps)
        graph_embed = graph_embed / (torch.norm(graph_embed, p=2, dim=1, keepdim=True) + eps)
        
        logits = torch.matmul(vision_embed, graph_embed.T) / self.temperature.exp()
        labels = torch.arange(logits.shape[0], device=self.device)
        
        contrastive_loss = (F.cross_entropy(logits, labels) + F.cross_entropy(logits.T, labels)) / 2.0
        
        # --- 组合总损失 ---
        loss = mask_loss + topo_loss + 0.01 * contrastive_loss
        
        self.log('train_mask_loss', mask_loss, on_step=True, on_epoch=False, prog_bar=True)
        self.log('train_topo_loss', topo_loss, on_step=True, on_epoch=False, prog_bar=True)
        self.log('train_contrastive_loss', contrastive_loss, on_step=True, on_epoch=False, prog_bar=True) # Log new loss
        self.log('train_loss', loss, on_step=True, on_epoch=False, prog_bar=True)
