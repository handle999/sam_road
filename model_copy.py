...
class SAMRoad(pl.LightningModule):
    ...
    def __init__(self, config):
        ...
        # ==========================================
        # [MODIFICATION 1]: 扩展 pixel_mean 和 std，支持 4 通道输入
        # 我们假设传入的第 4 通道 (Active Mask) 取值通常为 0 或 1 (或0~255已除以255)，
        # 所以将其均值设为 0，方差设为 1，不影响它的原始分布。
        ...
        if config.ENCODER_LORA:
            ...
            # lets freeze first
            # ==========================================
            # [MODIFICATION 3]: 强制解冻 Patch Embed 层
            # 因为第 4 通道是新加的，必须让网络训练这一层的权重来理解轨迹信息！
        ...
        with open(config.SAM_CKPT_PATH, "rb") as f:
            ...
            for k, v in self.named_parameters():
                ...
                # ==========================================
                # [MODIFICATION 4]: 拦截 patch_embed 权重的加载
                # 将 3 通道的预训练权重拷贝给前 3 个通道，第 4 通道保持全 0
                    ...
                    new_weight[:, :3, :, :] = old_weight # 拷贝 RGB 权重
                    # 第 4 通道 (new_weight[:, 3, :, :]) 保持全零初始化，实现无害化接入
                    matched_names.append(k)
    ...
    def forward(self, input_4c, graph_points, pairs, valid):
        # input_4c: [B, H, W, 4] -> RGB (3) + Active Prior (1)
        # graph_points: [B, N_points, 2]
        # pairs: [B, N_samples, N_pairs, 2]
        # valid: [B, N_samples, N_pairs]

        x = input_4c.permute(0, 3, 1, 2)
        # [B, C, H, W]
    ...
    def training_step(self, batch, batch_idx):
        ...
        # [B, H, W, C] while C = 4, RGB + Active Mask
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # masks: [B, H, W, C], C = 4, RGB + Active Mask,  topo: [B, N_samples, N_pairs, 1]
        mask_logits, mask_scores, topo_logits, topo_scores = self(rgb, graph_points, pairs, valid)
    ...
    def configure_optimizers(self):
        ...
        if self.config.ENCODER_LORA:
            ...
            encoder_params = {
                # ==========================================
                # [MODIFICATION 6]: 把 patch_embed 加入优化器！
                # 否则 LoRA 模式下只更新了 qkv，4通道的融合权重永远得不到训练
                'params': [p for k, p in self.image_encoder.named_parameters() if 'qkv.linear_' in k or 'patch_embed' in k],
                # ==========================================
