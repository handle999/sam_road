...
## 九、全面排查:test_completion 及所有坐标相关代码

### 9.1 test_completion 要不要修?

**不用单独修。** `test_completion.py` 本身不处理坐标——它只用 `SatMapCompletionDataset(is_train=False)` 跑 PR 曲线标定阈值。所有坐标逻辑都在 dataset 里,继承修复:
- `get_known_adj_for_render` 已转 `(x,y)` → render 对齐(Bug 2 训练侧修)
- `sample_patch` 的 `known_edge_index` 来自 igraph(本就 `(x,y)`,经 coord_transform)

验证:test_completion 用的 dataset `__getitem__`,`graph_points` 在 patch 内 `[0,256]`,`road_feature_map` 非零比例合理,`known_edge_index` 索引合法。坐标自洽。

### 9.2 所有坐标相关代码排查结果

| 位置 | 坐标系 | 状态 |
|------|--------|------|
| `coord_transform` 定义(dataset_completion L565/577/592) | `(r,c)→(x,y)` | ✓ 正确(didi/spacenet 分别处理) |
| `igraph_from_adj_dict`(graph_utils) | 入口转 `(x,y)` | ✓ 训练图结构正确 |
| `sample_patch` / `known_edge_set_subdivide` | `(x,y)`(igraph 空间) | ✓ 正确 |
| `map_known_edges_to_nms`(训练匹配) | `(x,y)` | ✓ 正确(用 subdivide_points,已转) |
| `get_known_adj_for_render`(训练 render 入口) | **已转 `(x,y)`** | ✓ 修复(Bug 2 训练侧) |
| `load_known_graph`(推理入口) | **已转 `(x,y)`** | ✓ 修复(Bug 1+2 推理侧) |
| `render_graph_feature_map`(训练+推理) | 期待 `(x,y)`,入口已转 | ✓ 修复(调用方传对了) |
| `_match_known_edges_to_graph_points`(推理匹配) | 期待 `(x,y)`,入口已转 | ✓ 修复 |
| `_inject_known_nodes`(推理注入) | 期待 `(x,y)`,入口已转 | ✓ 修复 |
| `generate_partial_prior.py` | 保存原始 `(r,c)`,PNG 用 transform_node | ✓ 正确(坐标转换由 load 入口负责) |
| `pred_nodes = graph_points[:, ::-1]`(L355,save) | `(x,y)→(r,c)` | ✓ 正确 |
| `pred_nodes = [400-r, c]`(L680,spacenet save) | `(r,c)→(y_up,x)` sat2graph | ✓ 正确(仅 spacenet) |
| `sam_road_completion.py` 模型内部 | 不碰坐标变换(只处理 tensor) | ✓ 正确 |
| `inferencer.py` / `inferencer_4ch.py` | 无 known graph 路径 | ✓ 不受影响 |

**结论:坐标 bug 已全覆盖,无遗漏。** 两个入口(`get_known_adj_for_render` + `load_known_graph`)转一次,所有下游消费点拿到的都是 `(x,y)`。`test_completion`、`generate_partial_prior`、模型内部都不需要改。

### 9.3 spacenet vs didi_xian 的差异(已统一处理)

`transform_known_graph_coords` 按数据集分支,与 `coord_transform` 完全对齐:

| 数据集 | pickle 坐标 | 原点 | 变换 |
|--------|------------|------|------|
| didi_xian / cityscale | `(row, col)` | 左上 | `(r,c)→(c,r)` |
| spacenet | `(raw_y, raw_x)` | 左下 | `(r,c)→(c, 400-r)` |

`generate_partial_prior.transform_node` 与 `transform_known_graph_coords` 验证一致(同一套变换,单一真相源)。

### 9.4 发现的独立预存问题(非坐标 bug,记录待评估)

排查中发现一个**与坐标无关的预存逻辑问题**:

`_create_known_graph`(dataset_completion L316-332)里,`known_edge_set_subdivide`(GNN 拓扑用)和 `known_edges_original`(render 用)是**两次独立随机采样**,不是同一组边。这意味着:
- GNN 看到的"已知边"和 render 画出的"已知路网"可能不一致
- 这是方案 B 设计文档里提过要修的"特征图一致性"问题,代码里仍是两次独立采样

**影响**:GNN 拓扑先验和 render 几何先验描述的不是同一个已知子图,可能削弱先验协同效果。但这不是坐标 bug,不影响本次修复的正确性,属独立问题,建议后续单独评估是否需要让两者共用一次采样。

### 9.5 修复后的完整验证清单

- [x] `transform_known_graph_coords` 单元测试:didi/spacenet 变换正确
- [x] Bug 1 边端点正确率:didi_xian/spacenet 0%→100%
- [x] Bug 2 render IoU:didi_xian 0.040→0.593,spacenet 0.019→0.609
- [x] `generate_partial_prior.transform_node` 与新函数一致
- [x] test_completion dataset 路径坐标自洽
- [x] save 链路(spacenet `400-r` 翻转)与注入节点自洽
- [x] 模型内部不碰坐标
- [x] extraction/4ch inferencer 无 known graph 路径,不受影响
